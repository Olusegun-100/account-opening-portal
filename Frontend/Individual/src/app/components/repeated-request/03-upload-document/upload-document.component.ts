...
export class UploadDocumentComponent {
  ...
  ngOnInit(): void {
    ...
    if(Object.keys(this.storage.rependingData).length < 1) {
      ...
      this.documentArray = this.storage.rependingDocuments;
    };    
    // this.rependingCustomerId = this.storage.rependingCustomerId;
  }  
  ...
  next () {
    this.storage.rependingDocuments = this.documentArray;
    // sessionStorage.setItem('documents', JSON.stringify(this.documentArray));
    this.router.navigate(['/r/review-information']);
  }

  back () {
    this.storage.rependingDocuments = this.documentArray;
    // sessionStorage.setItem('documents', JSON.stringify(this.documentArray));
    // let routeTo = this.rependingCustomerId;
    this.router.navigate(['/r/account-specification']);
  }

  imageLogo = this.storage.imageLogo;
